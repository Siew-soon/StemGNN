...
def inference(model, dataloader, device, node_cnt, window_size, horizon):
    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(dataloader):
            ...
            step = 0
            forecast_steps = np.zeros(
                [inputs.size()[0], horizon, node_cnt], dtype=np.float64
            )
            while step < horizon:
...
def validate(
    ...
    forecast_norm, target_norm = inference(
        model, dataloader, device, node_cnt, window_size, horizon
    )
    ...
    score = evaluate(target, forecast)  # Updated to use MSE evaluation
    score_by_node = evaluate(
        target, forecast, by_node=True
    )  # Updated to use MSE evaluation
    end = datetime.now()

    score_norm = evaluate(target_norm, forecast_norm)  # Updated to use MSE evaluation
    print(
...
def train(train_data, valid_data, args, result_file):
    ...
    for epoch in range(args.epoch):
        ...
        if (
            args.early_stop
            and validate_score_non_decrease_count >= args.early_stop_step
        ):
            break
